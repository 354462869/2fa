...
- 客户端生成不透明 `device_id`，通过 `POST /v1/devices` 注册，传入
  `{id, label}`。服务端原子性地创建设备记录并将当前会话绑定到该设备。
  响应仅返回 `Device` 对象（id、label、时间戳等），不返回新会话令牌。
- 首次连接时，扩展先登录（`POST /v1/auth/login`，不携带 `device_id`），
  并将获得的未绑定会话立即用于设备注册。为兼容旧客户端，服务端不限制未绑定
  会话只能调用注册接口；新版客户端应尽快完成注册，使后续撤销可按设备生效。
- 已绑定设备的客户端再次登录时应携带 `device_id`，服务端会验证设备归属。

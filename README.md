...
适合只想快速启动同步服务和管理 API 的场景。数据会保存在 Docker 卷
`2fa-data` 中，SQLite 数据库路径为 `/data/2fa.sqlite`。
...
首次访问管理后台时，如果数据库为空，会提示创建第一个管理员账号。生产环境请在
服务前放置 HTTPS 反向代理，并设置强随机 `SERVER_SESSION_SECRET`。

...
本版本完善服务端会话和设备撤销语义，增强扩展端凭据恢复与本地令牌保护，并将账号备注和创建时长改为延迟悬停浮层展示。

## 更新内容

- 服务端会话支持空闲续期和绝对生命周期上限；新增 `SERVER_SESSION_MAX_LIFETIME`，续期不会超过绝对期限。
- SQLite 会话表新增 `absolute_expires_at` 并在启动时幂等迁移，旧会话会以原到期时间完成回填。
- 设备注册改为原子性创建设备并绑定当前会话；设备缺失、归属不匹配或被撤销时，绑定会话立即失效。
- 认证 API 区分 `auth.session_expired`、`auth.session_invalid`、`auth.session_revoked` 和 `auth.user_disabled`，便于客户端执行准确恢复流程。
- 扩展远程会话令牌改为使用 IndexedDB 中不可导出的设备密钥加密保存；发现旧版明文令牌时会自动迁移。
- 扩展可识别登录过期、设备撤销和账号禁用状态，提供重新登录或设备重连入口，并保留原设备标识和标签。
- 账号卡片持续悬停 1 秒后显示备注和创建至今时长；浮层跟随指针、自动避让卡片和视口边界，并兼容短视口紧凑布局。
- 增加会话续期/绝对到期、设备撤销、设备注册事务、错误分类、令牌迁移以及 Chromium 恢复流程和悬停浮层的自动化测试。
- 主 release workflow 在上传资产前执行完整测试、构建、类型检查、Go vet 和 Docker 持久化 smoke；独立 Docker workflow 改为手动指定 tag，避免重复推送同一版本镜像。

## 兼容性和升级说明

- SQLite 迁移会在启动时自动执行且可重复运行；升级前仍应停止容器并备份完整 `/data`。
- `SERVER_SESSION_MAX_LIFETIME` 默认值为 `4320h`，必须大于或等于 `SERVER_SESSION_TTL`（默认 `720h`），否则服务端会拒绝启动。
- 旧客户端的未绑定会话继续兼容；新版扩展会在首次设备注册时绑定当前会话，使设备撤销能够即时生效。
- 已保存的扩展明文远程 token 会在首次读取时迁移为本地密文；服务端仍无法读取同步密码或扩展本地明文资料。
- 会话过期需要重新登录；设备被撤销后需要重新注册设备。Chromium 扩展需要重新加载新版构建产物。
- 管理 API 和服务端同步投影继续只暴露安全默认元数据，不返回 `secret_ciphertext`、`ct_b64`、私密备注或其他客户端加密正文。
- 反向代理部署必须覆盖客户端传入的 `X-Forwarded-For`，并禁止绕过代理直接访问服务端端口，以保证按 IP 认证限流有效。

## Release 资产

- 服务端 Docker 镜像：`ghcr.io/354462869/2fa-server:v0.2.3`
- 源码包：`2fa-source-v0.2.3.tar.gz`
- 浏览器扩展包：`2fa-extension-v0.2.3.zip`
...
本版本将项目从 TOTP/2FA 管理器演进为账号管理器（Account Manager），新增账号元数据、账号关系、管理后台关系图，并为后续用户手动触发网页填充保留 content script 基础能力。
...
- `accounts` / `relations` 增加 `created_at`，用于保存服务端同步元数据；扩展端可通过 `metadata_json` 携带客户端创建时间。
...
- 扩展新增 content script 的最小填充接收能力，但侧边栏账号卡片当前仍以复制操作为主，未开放静默自动填充。

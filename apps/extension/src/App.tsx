...
function getPlatformBadge(issuer: string) {
  ...
  return (
    <div className={`${baseClass} bg-gradient-to-tr ${gradient} text-white shadow-sm shadow-indigo-500/10`}>
      {initial}
    </div>
  );
}
...
export default function App() {
  ...
  const handleSaveItem = async (e: React.FormEvent) => {
    e.preventDefault();
    setErrorMessage(null);
    if (!newItemIssuer || !newItemSecret) {
      setErrorMessage('发行方和密钥为必填项');
      return;
    }
    ...
    try {
      ...
      const itemPlaintext = JSON.stringify({
        ...
        secret: newItemSecret.replace(/\s+/g, '').toUpperCase(),
        notes: newItemNotes
      });
      ...
      <div className="w-full h-screen min-w-[320px] flex flex-col justify-between bg-[#0B0F19] text-slate-100 overflow-hidden relative p-6">
        ...
        <div className="overflow-y-auto flex-1 no-scrollbar z-10">
          <div className="flex flex-col items-center text-center mt-6">
            <div className="w-14 h-14 bg-gradient-to-tr from-blue-600 to-violet-600 rounded-2xl flex items-center justify-center text-white mb-4 shadow-lg shadow-indigo-500/20">
              <Key className="w-7 h-7" />
            </div>
            <h1 className="text-xl font-bold bg-gradient-to-r from-white via-slate-100 to-slate-300 bg-clip-text text-transparent">AuthNest</h1>
            <h2 className="text-xs font-semibold text-indigo-400 mt-1">创建同步密码</h2>
            <p className="text-xs text-slate-400 mt-2 max-w-[280px] leading-relaxed">
              此密码用于在本地以及远程同步前加密您的 2FA 数据。
            </p>
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 text-[11px] text-amber-300 text-left mt-4 leading-relaxed max-w-[300px]">
              ...
              同步密码绝不会上传到任何服务器。如果您丢失了此密码，您加密的 2FA 密钥将无法恢复。
            </div>
          </div>
          ...
          <form onSubmit={handleSetupPassword} className="mt-6 space-y-4">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2 bg-slate-900/50 border border-slate-800/80 rounded-xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  required
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-blue-600 to-violet-600 hover:from-blue-500 hover:to-violet-500 text-white font-medium text-sm py-2.5 px-4 rounded-xl shadow-lg shadow-indigo-600/20 transition-all focus:ring-2 focus:ring-indigo-500 focus:outline-none"
            >
              初始化保管库
            </button>
          </form>
      ...
      <div className="w-full h-screen min-w-[320px] flex flex-col justify-between bg-[#0B0F19] text-slate-100 overflow-hidden relative p-6">
        ...
        <div className="overflow-y-auto flex-1 no-scrollbar z-10">
          <div className="flex flex-col items-center text-center mt-12">
            ...
            <h1 className="text-xl font-bold bg-gradient-to-r from-white via-slate-100 to-slate-300 bg-clip-text text-transparent">AuthNest</h1>
            <h2 className="text-xs font-semibold text-indigo-400 mt-1">保管库已锁定</h2>
            <p className="text-xs text-slate-400 mt-2 max-w-[260px] leading-relaxed">
              请输入您的同步密码以解密您的身份验证信息。
            </p>
          </div>
  ...
  return (
    <div className="w-full h-screen min-w-[320px] bg-[#0B0F19] text-slate-100 overflow-hidden flex flex-col font-sans select-none relative">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_bottom_left,rgba(124,58,237,0.08),transparent_50%)] pointer-events-none" />
      ...
      <div className="flex-1 flex flex-col min-h-0 z-10">
        ...
        <div className="bg-slate-900/40 backdrop-blur-md border-b border-slate-800/60 px-4 py-3 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 bg-gradient-to-tr from-blue-600 to-violet-600 rounded-lg flex items-center justify-center text-white shadow-sm shadow-indigo-500/25">
          ...
          <div className="flex items-center gap-1">
            {remoteConfig && (
              <button
                ...
                className="p-1.5 text-slate-400 hover:text-indigo-400 hover:bg-slate-800/50 rounded-lg transition-colors focus:outline-none focus:ring-1 focus:ring-indigo-500"
                title="立即同步"
              >
            ...
            <button
              ...
              className="p-1.5 text-slate-400 hover:text-rose-400 hover:bg-slate-800/50 rounded-lg transition-colors focus:outline-none focus:ring-1 focus:ring-indigo-500"
              title="锁定保管库"
            >
              <Lock className="w-3.5 h-3.5" />
            </button>
        ...
        <div className="w-full bg-slate-950 h-0.5 shrink-0">
          <div
            className={`h-full transition-all duration-1000 ease-linear ${
              secondsRemaining < 5 ? 'bg-rose-500' : 'bg-indigo-500'
            }`}
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-1.5 bg-slate-900/50 border border-slate-800/80 rounded-xl text-xs text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent placeholder-slate-500 backdrop-blur-md"
                  />
                </div>
                <button
                  onClick={() => {
                  ...
                  className="bg-gradient-to-r from-blue-600 to-violet-600 hover:from-blue-500 hover:to-violet-500 text-white p-1.5 rounded-xl transition-all flex items-center justify-center shadow-lg shadow-indigo-600/25 active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  title="添加账户"
                >
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-full whitespace-nowrap transition-colors focus:outline-none ${
                      selectedGroupId === null
                        ? 'bg-gradient-to-r from-blue-600 to-violet-600 text-white font-medium shadow-sm shadow-indigo-500/20'
                        ...
                        <div key={gid} className="space-y-2">
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] text-indigo-400 bg-indigo-500/10 px-2 py-0.2 rounded-full font-mono font-medium">
                                {itemsInGroup.length}
                              </span>
                            </div>
                            {isCollapsed ? (
                            ...
                            <div className="space-y-2">
                              {itemsInGroup.map((item) => {
                                ...
                                return (
                                  <div
                                    key={item.id}
                                    onClick={() => handleCopy(item.id, code.replace(/\s+/g, ''))}
                                    className="bg-slate-900/40 backdrop-blur-md border border-slate-800/80 hover:border-indigo-500/40 hover:bg-slate-900/60 rounded-2xl p-3 flex flex-col justify-between cursor-pointer transition-all duration-150 relative active:scale-[0.98] group"
                                  >
                                    <div className="flex items-start justify-between">
                                      ...
                                      <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                                        <button
                                          ...
                                          className="p-1 text-slate-400 hover:text-indigo-400 hover:bg-slate-800/60 rounded transition-colors focus:outline-none"
                                          title="编辑"
                                        >
                                      ...
                                      <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                        <div className="shrink-0 flex items-center justify-center" title={`剩余时间: ${secondsRemaining}秒`}>
                                          <svg height="16" width="16" className="transform -rotate-90">
                                            ...
                                            />
                                            <circle
                                              stroke={secondsRemaining < 5 ? '#f43f5e' : '#6366f1'}
                                              fill="transparent"
                                              strokeWidth="2"
                                        ...
                                        {isCopied ? (
                                          <span className="text-[9px] text-emerald-400 font-medium bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20 animate-fade-in">
                                            已复制
                                          </span>
                                        ) : (
                                          <button
                                            onClick={() => handleCopy(item.id, code.replace(/\s+/g, ''))}
                                            className="p-1 rounded bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 transition-all focus:outline-none"
                                            title="复制验证码"
                                          >
              ...
              {remoteConfig ? (
                <div className="space-y-3">
                  ...
                  <button
                    onClick={handleTriggerSync}
                    disabled={isSyncing}
                    className="w-full bg-gradient-to-r from-blue-600 to-violet-600 hover:from-blue-500 hover:to-violet-500 disabled:from-blue-600/50 disabled:to-violet-600/50 text-white font-medium text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-1.5 transition-all shadow-lg shadow-indigo-600/20 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                  >
                ...
                <form onSubmit={handleRemoteConnect} className="bg-slate-900/40 backdrop-blur-md border border-slate-800/80 rounded-2xl p-4 space-y-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-1.5 bg-slate-900/50 border border-slate-800/80 rounded-xl text-xs text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder-slate-600"
                      required
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-1.5 bg-slate-900/50 border border-slate-800/80 rounded-xl text-xs text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      required
                    />
                  </div>
                  <div>
              ...
              <form onSubmit={handleSaveSettings} className="bg-slate-900/40 backdrop-blur-md border border-slate-800/80 rounded-2xl p-4 space-y-3">
                ...
                <div className="flex items-start gap-2 pt-1">
                  <input
                    ...
                    className="mt-1 rounded border-slate-800 bg-slate-900 text-indigo-600 focus:ring-indigo-500 shrink-0"
                  />
                  <div>
                ...
                <button
                  type="submit"
                  className="w-full bg-gradient-to-r from-blue-600 to-violet-600 hover:from-blue-500 hover:to-violet-500 text-white font-medium text-xs py-2.5 px-4 rounded-xl shadow-lg shadow-indigo-600/20 transition-all focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                >
                  保存设置
                </button>
              </form>
        ...
        <div className="bg-slate-950/80 backdrop-blur-md border-t border-slate-800/85 flex justify-around py-1.5 shrink-0">
          <button
            ...
            className={`flex flex-col items-center gap-0.5 py-1 px-4 rounded-xl transition-all focus:outline-none ${
              activeTab === 'vault' ? 'text-indigo-400 bg-indigo-500/10 font-semibold' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/40'
            }`}
          >
            <Unlock className="w-4 h-4" />
            <span className="text-[9px]">验证码</span>
          </button>
          <button
            onClick={() => setActiveTab('groups')}
            className={`flex flex-col items-center gap-0.5 py-1 px-4 rounded-xl transition-all focus:outline-none ${
              activeTab === 'groups' ? 'text-indigo-400 bg-indigo-500/10 font-semibold' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/40'
            }`}
          >
            <Folder className="w-4 h-4" />
            <span className="text-[9px]">分组</span>
          </button>
          <button
            onClick={() => setActiveTab('sync')}
            className={`flex flex-col items-center gap-0.5 py-1 px-4 rounded-xl transition-all focus:outline-none ${
              activeTab === 'sync' ? 'text-indigo-400 bg-indigo-500/10 font-semibold' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/40'
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-0.5 py-1 px-4 rounded-xl transition-all focus:outline-none ${
              activeTab === 'settings' ? 'text-indigo-400 bg-indigo-500/10 font-semibold' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/40'
            }`}
          >
          ...
          <div className="relative flex-1 flex flex-col z-10">
            ...
            <form onSubmit={handleSaveItem} className="space-y-4 flex-1 flex flex-col justify-between">
              <div className="space-y-3.5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-900/50 border border-slate-800/80 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-900/50 border border-slate-800/80 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 uppercase font-mono"
                    required
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-2.5 py-1.5 bg-slate-900/50 border border-slate-800/80 rounded-xl text-xs text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 [&>option]:bg-[#0B0F19]"
                  >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 bg-slate-900/50 border border-slate-800/80 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                  />
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-800/60 mt-6">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-gradient-to-r from-blue-600 to-violet-600 hover:from-blue-500 hover:to-violet-500 text-white py-2 rounded-xl text-xs font-medium focus:outline-none shadow-md shadow-indigo-600/25"
                >
                  保存
                </button>
              </div>
